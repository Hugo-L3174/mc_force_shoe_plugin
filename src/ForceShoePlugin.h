...
#define EXIT_ON_ERROR(res,comment) if (res != XRV_OK) { mc_rtc::log::error_and_throw("Error {} occurred in " comment ": {}\n",res,xsensResultText(res)); exit(1); }
...
struct ForceShoePlugin : public mc_control::GlobalPlugin
{
  ...
  void UnloadedFS()
  {
    ...
    while (res == XRV_OK && sdata_ < 99)
    {
      ...
      for (int i = 0; i < 8; i++)
      {
        // calculation is: position of the first (index 0) calibrated acceleration data + offset of calibrated data + offset of rawforce data + size of a short (2bytes)
