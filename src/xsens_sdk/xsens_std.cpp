...
const char* xsensResultText(const XsensResultValue result)
{
	switch(result)
	{
		...
		case XRV_MEASUREMENTFAIL4:		return "Timer overflow during measurement – MT does not respond within measurement period. Increase period (lower sample frequency)";
		case XRV_MEASUREMENTFAIL5:		return "Timer overflow during measurement – MT response was not received within measurement period. Increase period (lower sample frequency) or use fewer Motion Trackers";
